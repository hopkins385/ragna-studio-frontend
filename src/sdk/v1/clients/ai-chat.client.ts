...
import { BaseApiClient } from '@/sdk/v1/base/base-api.client';
...
export class AiChatClient extends BaseApiClient {
  ...
  public async deleteAllChatMessages(payload: { chatId: InputChatId }) {
    ...
    const { status } = await this.client.DELETE().setRoute(route).setSignal(this.ac.signal).send();
